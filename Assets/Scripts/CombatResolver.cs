...
public static class CombatResolver
{
    ...
    struct Fighter
    {
        ...
        public float critDamage;     // ADDITIONAL to normal damage, e.g. 50 = 1.5x total
        public float speed;
    ...
    /// <summary>
    /// Resolve combat and return a detailed turn-by-turn log for visual playback.
    /// Same deterministic logic as Resolve(), but records every action.
    /// </summary>
    ...
    /// <summary>
    /// Execute one attack, log it, and return true if combat is over.
    /// </summary>
    ...
    {
        float auraMult = 1f + ((player.level - 1) * 1.5f + player.bonusAuraPercent) / 100f;
        float classDmgMult = player.playerClass == PlayerClass.Magier ? 1.25f : 1f;
        ...
        return new Fighter
        {
            ...
            weaponDmgMax = Mathf.Max(1, player.bonusWeaponDmgMax),
            mainStatValue = player.GetEffectiveMainStat(),
            levelDmgMult = (1f + player.level * 0.03f) * auraMult * classDmgMult,
            flatDamage = 0,
            armor = player.armor,
            armorCap = player.GetArmorCap(),
            critRate = player.critRate,
    ...
    /// <summary>
    /// Calculate damage (original, no crit output).
    /// </summary>
    ...
    static long CalculateDamage(Fighter attacker, Fighter defender, out bool isCrit)
    {
        ...
        // critDamage is ADDITIONAL to normal damage (e.g. 50% = 1.5x total)
